...
public class EvolvedBasicStrategyCountingInversExceptions extends PlayerStrategy {
	public EvolvedBasicStrategyCountingInversExceptions(CardDeck deck) {
		super(deck, "EvolvedBasicStrategy with Invers Exceptions", true);
		...
		while ((value = deck.countValueBeneficial(cards)) < 17) {
			...
			if (cards.remove(Integer.valueOf(11))) {
				...
				if (additionalValue < 11 && additionalValue > 1) {
					String[] conditionalAction = mapCounts.get("A" + additionalValue + " " + dealer.openCard());
					if (conditionalAction != null)
						if (-deck.getTrueCount() >= Integer.parseInt(conditionalAction[0]))
							action = conditionalAction[1];
						else action = conditionalAction[2];
